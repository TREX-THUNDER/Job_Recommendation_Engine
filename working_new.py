        ...
        if is_employed:
            ...
            for col in emp_columns + ['currentCompany', 'annualSalary']:
                if col in user_info and pd.notna(user_info[col]) and str(user_info[col]).lower() != 'null':
                    st.markdown(f"• <strong>{col.capitalize()}:</strong> {user_info[col]}", unsafe_allow_html=True)
        else:
            st.markdown('<p class="small-font">Currently not employed</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
